"""
Modified version of mlm_pytorch from https://github.com/lucidrains/mlm-pytorch
"""
import math
...
def mask_with_tokens(t, token_ids, stop_mask_idx=None):
    ...
    if stop_mask_idx:
        ...
        if torch.cuda.is_available():
            idx = idx.cuda()
        stop_mask_idx = torch.unsqueeze(stop_mask_idx, -1)
        stop_mask = idx.tile((*t.shape[:-1], 1))
        stop_mask = stop_mask >= stop_mask_idx
        mask |= stop_mask
    return mask
...
class MLM(nn.Module):
    ...
    def forward(self, input, stop_mask_idx=None, **kwargs):
        # do not mask [pad] tokens, or any other tokens in the tokens designated to be excluded ([cls], [sep])
        # also do not include these special tokens in the tokens chosen at random
        no_mask = mask_with_tokens(input, self.mask_ignore_token_ids, stop_mask_idx)
